  ...
  <AppShell>
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="r in reports" :key="r.name" class="border-b border-gray-50 hover:bg-gray-50 cursor-pointer" @click="goToDetail(r.name)">
...
import { FileText, Clock, CheckCircle, Link as LinkIcon, Search, Upload, Plus } from 'lucide-vue-next'
...
const CHANNEL_COLORS = { Mobile: 'bg-red-50 text-red-600', SMS: 'bg-green-50 text-green-600', USSD: 'bg-amber-50 text-amber-600', WhatsApp: 'bg-gray-100 text-gray-600' }
...
function goToDetail(id) { router.push(`/case-detail?id=${id}`) }
...
async function loadReports() {
  reports.value = await getList(
    'Case Report',
    ['name', 'reporter_name', 'symptom_tags', 'location_name', 'channel', 'status', 'report_date'],
