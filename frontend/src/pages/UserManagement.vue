      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="u in pagedUsers" :key="u.name" class="border-b border-gray-50 hover:bg-gray-50">
            ...
            <td class="px-5 py-3 text-gray-700">{{ regionLabel(u.assigned_region) }}</td>
...
import { Search, Plus, Pencil, Trash2, X, UserRound, Users, CheckCircle2, UserX, Ban } from 'lucide-vue-next'
import AppShell from '@/components/layout/AppShell.vue'
import { getUsers, createUser, updateUser, deleteUser, getRoles, getRegions, createRole } from '@/api/frappe'
...
const roleError = ref('')
const newRole = ref('')
const emptyForm = () => ({ email: '', first_name: '', last_name: '', primary_role: '', assigned_region: '', account_status: 'Active', password: '' })
const form = reactive(emptyForm())
const lastCreatedPassword = ref('')

// Builds a quick lookup so the table can show "Dadaab" instead of the raw Region ID
const regionMap = computed(() => Object.fromEntries(regions.value.map((r) => [r.name, r.region_name])))
function regionLabel(id) {
  if (!id) return '\u2014'
  return regionMap.value[id] || id
}
...
const pagedUsers = computed(() => filteredUsers.value.slice(page.value * PAGE_SIZE, page.value * PAGE_SIZE + PAGE_SIZE))
...
function openEditUser(u) {
  ...
  formError.value = ''
  Object.assign(form, {
    email: u.name, first_name: u.first_name, last_name: u.last_name || '',
    primary_role: u.primary_role || '', assigned_region: u.assigned_region || '', account_status: u.account_status || 'Active',
  })
  showUserModal.value = true
}
